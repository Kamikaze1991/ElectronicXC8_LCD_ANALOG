...
void TFT_InvertDisplay(_Bool i);
...
void TFT_DrawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color)
{
    if(x0 == x1){
        ...
        TFT_DrawVLine(x0, y0, y1 - y0 + 1, color);
    }else if(y0 == y1){
        if(x0 > x1) { int16_t t = x0; x0 = x1; x1 = t; };
        TFT_DrawHLine(x0, y0, x1 - x0 + 1, color);
    }else{
...
void TFT_FillCircleHelper(uint16_t x0, uint16_t y0, uint16_t r, uint8_t corners, uint16_t delta, uint16_t color)
{
    ...
    while(x < y) {
        ...
        f += ddF_x;
        if(x < (y + 1)){
            if(corners & 1) TFT_DrawVLine(x0+x, y0-y, 2*y+delta, color);
            if(corners & 2) TFT_DrawVLine(x0-x, y0-y, 2*y+delta, color);
        }
        if(y != py){
            if(corners & 1) TFT_DrawVLine(x0+py, y0-px, 2*px+delta, color);
            if(corners & 2) TFT_DrawVLine(x0-py, y0-px, 2*px+delta, color);
            py = y;
        }
        px = x;
    }
...
void TFT_DrawRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t r, uint16_t color)
{
    ...
    TFT_DrawHLine(x+r, y, w-2*r, color);
    TFT_DrawHLine(x+r, y+h-1, w-2*r, color);
    TFT_DrawVLine(x, y+r, h-2*r, color);
    TFT_DrawVLine(x+w-1, y+r, h-2*r, color);
...
void TFT_Print(uint8_t c)
{
    ...
    for(uint8_t i = 0; i < 5; i++ ){
        ...
        for(uint8_t j = 0; j < 8; j++, line >>= 1){
            if(line & 1) {
                if(textsize == 1)
                    TFT_DrawPixel(cursor_x + i, cursor_y + j, textcolor);
                else
                    TFT_FillRect(cursor_x + i * textsize, cursor_y + j * textsize, textsize, textsize, textcolor);
            }
            else{
                if(textbgcolor != textcolor) {
                    if(textsize == 1)
                        TFT_DrawPixel(cursor_x + i, cursor_y + j, textbgcolor);
                    else
                        TFT_FillRect(cursor_x + i * textsize, cursor_y + j * textsize, textsize, textsize, textbgcolor);
                }
            }
        }
    }
    if(textbgcolor != textcolor){
        if(textsize == 1) TFT_DrawVLine(cursor_x + 5, cursor_y, 8, textbgcolor);
        else TFT_FillRect(cursor_x + 5 * textsize, cursor_y, textsize, 8 * textsize, textbgcolor);
    }
    cursor_x += textsize * 6;
